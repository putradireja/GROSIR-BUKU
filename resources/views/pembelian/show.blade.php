...
<div id="printArea">
<x-card title="Detail Pembelian (Barang Masuk)" subtitle="Informasi lengkap transaksi penerimaan barang">
    {{-- Info Utama & Supplier --}}
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
        <div class="rounded-2xl bg-purple-50/50 p-4">
            <dl class="space-y-2">
                <div class="flex items-center gap-3">
                    <dt class="w-32 text-xs font-bold uppercase tracking-wide text-purple-400">No. Pembelian</dt>
                    <dd class="text-sm font-semibold text-slate-700">{{ $pembelian->no_beli }}</dd>
                </div>
                <div class="flex items-center gap-3">
                    <dt class="w-32 text-xs font-bold uppercase tracking-wide text-purple-400">Tanggal</dt>
                    <dd class="text-sm text-slate-700">{{ \Carbon\Carbon::parse($pembelian->tgl_beli)->format('d F Y') }}</dd>
                </div>
                <div class="flex items-center gap-3">
                    <dt class="w-32 text-xs font-bold uppercase tracking-wide text-purple-400">Ref PO</dt>
                    <dd class="text-sm text-slate-700">{{ $pembelian->pemesanan->no_pesan ?? 'Tanpa PO' }}</dd>
                </div>
                <div class="flex items-center gap-3">
                    <dt class="w-32 text-xs font-bold uppercase tracking-wide text-purple-400">Tipe / Status</dt>
                    <dd class="flex items-center gap-2">
                        <x-badge :color="$pembelian->tipe == 'cash' ? 'blue' : 'red'">{{ strtoupper($pembelian->tipe) }}</x-badge>
                        <x-badge :color="$pembelian->status_bayar == 'lunas' ? 'green' : 'warning'">{{ strtoupper($pembelian->status_bayar) }}</x-badge>
                    </dd>
                </div>
                @if($pembelian->tipe == 'credit')
                <div class="flex items-center gap-3">
                    <dt class="w-32 text-xs font-bold uppercase tracking-wide text-purple-400">Jatuh Tempo</dt>
                    <dd class="text-sm font-bold text-red-600">{{ \Carbon\Carbon::parse($pembelian->jatuh_tempo)->format('d F Y') }}</dd>
                </div>
                @endif
            </dl>
        </div>
    ...
    {{-- Tabel Barang --}}
    <h6 class="text-sm font-semibold text-slate-700 mb-3">Daftar Barang Diterima</h6>
    <div class="overflow-x-auto rounded-xl border border-purple-100">
        <table class="table-premium w-full text-left">
            ...
            <tfoot class="bg-purple-50/50">
                <tr>
                    <td colspan="4" class="text-right font-bold text-purple-700">GRAND TOTAL:</td>
                    <td class="text-right font-bold text-green-700 text-lg">Rp {{ number_format($pembelian->total, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    <div class="mt-6 flex items-center gap-3 border-t border-purple-50 pt-5 no-print">
        ...
        <x-button as="a" href="{{ route('pembelian.index') }}" variant="ghost">Kembali</x-button>
    </div>
</x-card>
</div>
@endsection